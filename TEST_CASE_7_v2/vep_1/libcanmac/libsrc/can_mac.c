...
#include "settings.h"
  ...
  void can_current_state_initial(CAN_CHECK* current_state)
  {
...
{
  if(data_sym == DOMINANT)
  {
    current_state->rec = 0;
    current_state->dom++;
    if(current_state->dom >= 5)
    {
      bool crc = current_state->crc_calc_check;
      bool arb = current_state->arbitration_check;
      current_state->crc_calc_check = false;
      current_state->arbitration_check = false;
      send(RECESSIVE, current_state);
      current_state->crc_calc_check = crc;
      current_state->arbitration_check = arb;
    }
  }
  else if (data_sym == RECESSIVE)
  {
    current_state->dom = 0;
    current_state->rec++;
    if(current_state->rec >= 5)
    {
      bool crc = current_state->crc_calc_check;
      bool arb = current_state->arbitration_check;
      current_state->crc_calc_check = false;
...
#if(defined(TEST_CASE_7))
bool calc_send_bitstuff(CAN_SYMBOL data_sym, CAN_CHECK* current_state)
{
  if(data_sym == DOMINANT)
  {
    current_state->dom++;
...
void receive(CAN_SYMBOL* data_sym, CAN_CHECK* current_state)
{
  ...
  if(current_state->bitstuff_check)
  {
    if(!calc_send_bitstuff(*data_sym, current_state))
    {
      bool crc = current_state->crc_calc_check;
      bool arb = current_state->arbitration_check;
      current_state->crc_calc_check = false;
...
#elif(defined(TEST_CASE_5) || defined(TEST_CASE_6) || defined(TEST_CASE_7))
...
void wait_for_dominant()
{
  ...
  bool can_mac_tx_frame(CAN_FRAME* txFrame)
  {
    ...
    bool success = true;
    can_current_state_initial(current_state);
