...
int main (void)
{
 ...
 uint64_t t = read_global_timer();
  ...
  while(true)
  {
    ...
    frame_1->ID = 0;
    frame_1->DLC = 7;
    frame_1->Data = 3000000000;
    frame_1->CRC = 0x80003333;


    uint64_t framesendend_1 = read_global_timer()+PERIOD0;
    xil_printf("CAN client %d %d Sending frame id: 0x%x, DLC: 0x%x, Data: 0x%08x%08x, CRC: 0x%x\n", TILE_ID, PARTITION_ID, frame_1->ID, frame_1->DLC, (uint32_t)(frame_1->Data >> 32), (uint32_t)frame_1->Data, frame_1->CRC);

    while(!can_mac_tx_frame(frame_1)){
      if(read_global_timer() > framesendend_1 - (FRAME_LENGTH_CYCLES + SYMBOL_LENGTH_CYCLES * (25 + 8*frame->DLC + 44 + ((34 + 8*frame->DLC - 1) / 4))))
      {
        xil_printf("Did not acknowladge in time, cannot resend!\n");
        break;
      }
      xil_printf("Acknoeladge failed , Resending!\n");
      //SET NOT PRESET SO IT IS CALCULATED
      frame->CRC = 69;
    }
      busy_wait_till(framesendend_1);
    //Send preset CRC
    frame->CRC = 0xB0003333;
    xil_printf("CAN client %d %d Sending frame id: 0x%x, DLC: 0x%x, Data: 0x%08x%08x, CRC: 0x%x\n", TILE_ID, PARTITION_ID, frame_1->ID, frame_1->DLC, (uint32_t)(frame_1->Data >> 32), (uint32_t)frame_1->Data, frame_1->CRC);
  }      
