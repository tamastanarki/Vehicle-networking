...
int main (void)
{
  ...
  /******* insert CAN Client code for TEST_CASE_1 here *******/
while(true)
  {
  ...
  /******* insert CAN Client code for TEST_CASE_2 here *******/
while(true)
  {
    CAN_FRAME* frame = (CAN_FRAME*)malloc(sizeof(CAN_FRAME));
    frame->ID = 0;
...
#elif defined(TEST_CASE_4) || defined(TEST_CASE_4)
  uint64_t t = read_global_timer();
  xil_printf("%04u/%010u: CAN client %d %d running in TEST_CASE_4\n", (uint32_t)(t >> 32), (uint32_t)t, TILE_ID, PARTITION_ID);
  /******* insert CAN Client code for TEST_CASE_4 and 5 here *******/
while(true)
  {
    CAN_FRAME* frame = (CAN_FRAME*)malloc(sizeof(CAN_FRAME));
    frame->ID = 0;
