...
void calc_curr_crc_bit(CAN_SYMBOL data_sym, CAN_CHECK* current_state){
	...
	if(current_state->crc_tracker & 0x8000){
		
		
		current_state->crc_tracker = CRC_BASE^current_state->crc_tracker;
...
bool can_mac_tx_frame(CAN_FRAME* txFrame)
{
  ...
  else
  {
	...
	send_decon_uint32(txFrame->crc_tracker,15, current_state);
	}
...
void can_mac_rx_frame(CAN_FRAME* rxFrame)
{
  ...
  send(DOMINANT, current_state);                           //IDE
  send(DOMINANT, current_state);                           //r0
  send_decon_uint32(rxFrame->DLC, 4, current_state);                 //DLC
  send_decon_uint64(rxFrame->Data, rxFrame->DLC * 8, current_state); //Data depending on DLC
  send_decon_uint32(rxFrame->CRC,15, current_state); //crc
